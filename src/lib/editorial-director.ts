...
export type EditorialBeat = {
  ...
  visualAction: string;
  audioAction: string;
  captionAction: "normal" | "reduced" | "off";
  emphasis: "low" | "medium" | "high";
  callbackSceneId?: string;
};

export type EditorialDirection = {
  version: "editorial-director-1";
  generatedAt: string;
...
function classifyBeat(scene: Scene, index: number): EditorialBeatType {
  if (index === 0) return "cold_open";
  if (scene.kind === "value_swap" || scene.kind === "confidence") return "reveal";
  if (
    scene.kind === "document" ||
  ...
  if (scene.kind === "map_story" || scene.kind === "data_chart") return "context";
  if (scene.kind === "quote") return "breath";
  if (scene.kind === "cta") return "cta";
  if (scene.kind === "diagram" || scene.kind === "timeline") return "transition";
  return index >= 7 ? "payoff" : "establish";
}

function splitCount(scene: Scene, role: ShotInstruction["role"]) {
  if (scene.durationSec <= 14) return 1;
  if (role === "broll" || role === "archive") {
    return Math.min(4, Math.max(2, Math.round(scene.durationSec / 7)));
  }
  if (role === "map" || role === "chart" || role === "diagram") {
    return Math.min(3, Math.max(2, Math.round(scene.durationSec / 10)));
  }
  if (scene.durationSec > 28) return 2;
  return 1;
}

function visualAction(
  beatType: EditorialBeatType,
  shot: ShotInstruction,
  part: number,
  total: number
) {
  if (beatType === "cold_open") {
    return part === 0
...
function audioAction(beatType: EditorialBeatType, part: number) {
  switch (beatType) {
    case "cold_open":
      return part === 0
...
function captionAction(beatType: EditorialBeatType) {
  if (beatType === "breath") return "off" as const;
  if (beatType === "reveal") return "reduced" as const;
  return "normal" as const;
}

function emphasis(beatType: EditorialBeatType) {
  if (beatType === "cold_open" || beatType === "reveal" || beatType === "payoff") {
    return "high" as const;
  }
  if (beatType === "breath" || beatType === "evidence") return "medium" as const;
  return "low" as const;
}
...
function callbackTarget(
  ...
  const earlyReveal = project.scenes.find(
    (candidate) =>
      candidate.kind === "value_swap" ||
      candidate.kind === "document" ||
      candidate.kind === "confidence"
  );

  if (scene.kind === "quote" || scene.kind === "cta") {
    return earlyReveal?.id || hook?.id;
  }
...
export function buildEditorialDirection(
  ...
  const production = buildProductionIntelligence(project);
  const shotByScene = new Map(
    production.shots.map((shot) => [shot.sceneId, shot])
  );

  const beats: EditorialBeat[] = [];

  project.scenes.forEach((scene, sceneIndex) => {
    const shot = shotByScene.get(scene.id);
    if (!shot) return;

    const type = classifyBeat(scene, sceneIndex);
    const parts = splitCount(scene, shot.role);
    const partDuration = scene.durationSec / parts;
    const start = sceneStart(project, sceneIndex);
    const callbackSceneId = callbackTarget(project, scene, sceneIndex);

    for (let part = 0; part < parts; part += 1) {
      const localType =
        callbackSceneId && part === parts - 1 && scene.kind !== "cta"
          ? "callback"
          : type;

      beats.push({
        id: `${scene.id}-beat-${part + 1}`,
        sceneId: scene.id,
        type: localType,
        startSec: Number((start + partDuration * part).toFixed(2)),
        durationSec: Number(partDuration.toFixed(2)),
        shotRole: shot.role,
        instruction:
          localType === "callback"
        ...
        visualAction:
          localType === "callback"
            ...
            : visualAction(type, shot, part, parts),
        audioAction: audioAction(type, part),
        captionAction: captionAction(type),
        emphasis: emphasis(type),
        callbackSceneId:
          localType === "callback" ? callbackSceneId : undefined,
      });
    }
  });

  const durations = beats.map((beat) => beat.durationSec);
  const averageShotLengthSec = durations.length
    ? durations.reduce((a, b) => a + b, 0) / durations.length
    : 0;

  const quietBeatCount = beats.filter(
    (beat) => beat.type === "breath" || beat.captionAction === "off"
  ).length;

  const visualResetCount = production.rhythm.filter(
    (item) => item.reset
  ).length;

  const warnings = [...production.warnings];

  if (averageShotLengthSec > 10) {
    warnings.push(
      "Average editorial beat is long. Consider more shot variation in B-roll and archival sections."
    );
  }

  if (!quietBeatCount && project.scenes.length >= 8) {
    warnings.push(
  ...
  if (
    beats.filter((beat) => beat.type === "callback").length === 0 &&
  ...
  const score = Math.max(
    ...
    Math.min(
      100,
      Math.round(
        production.score * 0.65 +
          Math.min(15, visualResetCount * 2) +
          Math.min(10, quietBeatCount * 5) +
          Math.min(
            10,
            beats.filter((beat) => beat.type === "callback").length * 5
          ) -
          warnings.length * 2
      )
    )
  ...
  return {
    ...
    beats,
    pacing: {
      averageShotLengthSec: Number(averageShotLengthSec.toFixed(2)),
      longestBeatSec: Number(
        (durations.length ? Math.max(...durations) : 0).toFixed(2)
      ),
      visualResetCount,
      quietBeatCount,
    },
